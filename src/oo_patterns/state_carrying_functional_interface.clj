(ns oo-patterns.state_carrying-functional-interface)

;; custom comparator, basically take a list of comparators and apply them
(defn make-composed-comparison
  [& comparisons]
  (fn [p1 p2]
  (let [results (for [comparison comparisons] (comparison p1 p2))
        first-non-zero-result (some (fn [result] (if (not (= 0 result))
                                                  result
                                                  nil)) results)]
